...
class SelfAttentionBlock(nn.Module):
    ...
    def _forward_list(self, x_list: List[Tensor], rope_list=None) -> List[Tensor] | Tuple[List[Tensor]]:
        """
        ...
        if self.training and self.sample_drop_ratio > 0.0:
            ...
            for x, rope in zip(x_list, rope_list):
                x_attn_c = self.ls1(self.attn(self.norm1(x), rope=rope))
                # clear
                x_ffn_c = x_attn_c
                # fusion
                x_attn = x + x_attn_c
                ...
                x_out.append(x_ffn)
                x_c_out.append(x_ffn_c)
            x_ffn = x_out

        return x_ffn, x_c_out

    def forward(self, x_or_x_list, rope_or_rope_list=None) -> List[Tensor] | Tuple[List[Tensor]]:
        if isinstance(x_or_x_list, Tensor):
            # for reference:
            # return self._forward(x_or_x_list, rope=rope_or_rope_list)
            # in order to match implementations we call the list op:
            x_out, x_c_out = self._forward_list([x_or_x_list], rope_list=[rope_or_rope_list])
            return x_out[0], x_c_out[0]
